...
- **7 個 Hermes 工具**：`expense_add`（新增）、`expense_list`（查詢）、`expense_report`（報表）、`expense_delete`（刪除）、`expense_edit`（編輯）、`expense_search`（搜尋）、`expense_config`（設定）
...
插件在 `couple-finance` 工具集下註冊了 7 個工具：
...
| `expense_edit` | PATCH 風格的部份更新；回傳更新後的記錄以及變更的欄位 diff |
...
> 使用者：「把晚餐改成 950，類別改為餐飲」  
> Agent 呼叫 `expense_edit(expense_id=3, amount=950, category="dining")` → 回傳更新記錄 + `{"diff": {"amount": {"old": 850.0, "new": 950.0}, "category": {"old": "交通", "new": "餐飲"}}}`
...
plugins/couple-finance/
  plugin.yaml          # 插件中繼資料
  __init__.py          # 進入點：register() + 7 個工具處理函式
  db.py                # SQLite 結構、連線、CRUD（13 個公開函式）
...
- **資料表**：`expenses`（軟刪除透過 `is_deleted` 標記；另有 `updated_at`、`edit_reason` 欄位供編輯使用）、`config`（鍵值設定）
...
- `expense_edit` 是 PATCH 風格的部份更新 — 只會更新有提供的欄位；回應中會包含 `"diff"` 物件，只顯示實際有變更的內容
